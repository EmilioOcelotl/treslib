...
export class SnapToGrains {
    constructor(audioContext, grainEngine, options = {}) {
        ...
        this.smoothingTime = options.smoothingTime ?? 1.0; // Más suavizado
        this.maxRandomPitch = options.maxRandomPitch ?? 0.3;
        this.maxRandomPosition = options.maxRandomPosition ?? 0.01;
        this.pointerTransitionTime = options.pointerTransitionTime ?? 3.0; // Segundos entre transiciones
        this.transitionCurve = options.transitionCurve ?? 'easeInOut'; // easeInOut, linear, exponential
    ...
    generatePointerSequence(analysis) {
        ...
        // Suavizar adicionalmente la secuencia
        this.pointerSequence = this.smoothSequence(basePattern);
        
        console.log('Secuencia suavizada:', this.pointerSequence);
    }
    ...
    // --------------------------
    // Análisis del snapshot (mantener igual)
    // --------------------------
...
mapSnapshotToAudioParams(analysis) {
    ...
    applyToGrainEngine(params) {
        ...
        // Aplicar con transiciones suaves
        this.grainEngine.setRate(params.rate, now + fadeTime);
        this.grainEngine.setAmp(params.amp, now + fadeTime);
